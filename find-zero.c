...
#define DEFAULT_IGNORE_XTIME 1
#define DEFAULT_ZEROFILE_ONLY 1
#define DEFAULT_XFER_MODE 1
...
char buf[CHUNK];
static char zero[CHUNK];

int
is_zerofile (struct dirjob *job, struct xdirent *entry)
{
	char path[1024];
	int  fd = -1;
	size_t size = 0;
	...
	if (!S_ISREG(entry->xd_stbuf.st_mode))
		return 0;

	if (entry->xd_stbuf.st_size == 0)
		return 1;

	if (entry->xd_stbuf.st_blocks == 0)
		return 1;

	sprintf (path, "%s/%s", job->dirname, entry->xd_name);
...
xworker_do_xfer (struct xwork *xwork, struct list_head *jobs)
{
	...
	list_for_each_entry (job, jobs, list) {
		list_for_each_entry (entry, &job->files, list) {
			if (ZEROFILE_ONLY && !is_zerofile (job, entry))
				continue;
...
parse_env (void)
{
	...
	if (setenvint ("REPLICA", &REPLICA) == -1) {
		tdbg ("Defaulting REPLICA to 1\n");
		REPLICA = 1;
	}

	if (setenvint ("INDEX", &INDEX) == -1) {
		tdbg ("Defaulting INDEX to 1\n");
		INDEX = 1;
	}

	if (setenvint ("WORKERS", &WORKERS) == -1) {
		tdbg ("Defaulting WORKERS to %d\n", DEFAULT_WORKERS);
		WORKERS = DEFAULT_WORKERS;
	}

	if (setenvstr ("XFER_CMD", &XFER_CMD) == NULL) {
		tdbg ("Defaulting XFER_CMD to " DEFAULT_XFER_CMD "\n");
		XFER_CMD = DEFAULT_XFER_CMD;
	}

	if (setenvint ("IGNORE_XTIME", &IGNORE_XTIME) == -1) {
		tdbg ("Defaulting IGNORE_XTIME to %d\n", DEFAULT_IGNORE_XTIME);
		IGNORE_XTIME = DEFAULT_IGNORE_XTIME;
	}

	if (setenvint ("ZEROFILE_ONLY", &ZEROFILE_ONLY) == -1) {
		tdbg ("Defaulting ZEROFILE_ONLY to %d\n", DEFAULT_ZEROFILE_ONLY);
		ZEROFILE_ONLY = DEFAULT_ZEROFILE_ONLY;
	}
	...
	if (setenvint ("MB_PER_TAR", &MB_PER_TAR) == -1) {
		tdbg ("Defaulting MB_PER_TAR to 32\n");
		MB_PER_TAR = 32;
	}

	if (setenvtime ("DEFAULT_STIME", &DEFAULT_STIME) == -1) {
		tdbg ("Setting DEFAULT_STIME to %d\n", 0);
		DEFAULT_STIME = 0;
	}
